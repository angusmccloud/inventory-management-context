...
# =============================================================================
# Aggregates rules from constitution, AGENTS.md, and shared context into a
# comprehensive copilot-instructions.md file for GitHub Copilot code review.
#
# Usage: .github/scripts/generate-copilot-instructions.sh [output_file]
#
# Output: Creates .github/copilot-instructions-aggregated.md (or specified file)
# =============================================================================
...
# Output file (can be overridden by first argument)
OUTPUT_FILE="${1:-.github/copilot-instructions-aggregated.md}"
...
# Extract NON-NEGOTIABLE principles from constitution
extract_principles() {
    if [[ -f "$CONSTITUTION_FILE" ]]; then
        # Extract content between "## Core Principles" and "## Technology Stack"
        sed -n '/^## Core Principles/,/^## Technology Stack/p' "$CONSTITUTION_FILE" | head -n -1
    fi
}
...
extract_tech_stack() {
    if [[ -f "$CONSTITUTION_FILE" ]]; then
        # Extract content between "### Core Technologies" and "### Development Standards"
        sed -n '/^### Core Technologies/,/^### Development Standards/p' "$CONSTITUTION_FILE" | head -n -1
    fi
}

# Extract development standards from constitution
extract_dev_standards() {
    if [[ -f "$CONSTITUTION_FILE" ]]; then
        # Extract content between "### Development Standards" and "## Development Workflow"
        sed -n '/^### Development Standards/,/^## Development Workflow/p' "$CONSTITUTION_FILE" | head -n -1
    fi
}

# Extract shared context (manual additions section)
extract_shared_context() {
    if [[ -f "$SHARED_CONTEXT_FILE" ]]; then
        # Extract content after "## Manual Additions"
        sed -n '/^## Manual Additions/,$p' "$SHARED_CONTEXT_FILE" | tail -n +3
    fi
}

# Generate the aggregated instructions
